...
from evaluate import PredictGrid
...
class Trainer:
    ...
    def _do_epoch(self):
        ...
        for it, batch in enumerate(self.train_loader):
            ...
            self.clearml_logger.report_scalar(
                f"train_{trainFold}", "main", iteration=it, value=loss_dict["main"]
            )
            self.clearml_logger.report_scalar(
                f"train_{trainFold}", "total", iteration=it, value=loss_dict["total"]
            )
    ...
    def do_training(self):
        self.logger = Logger(
            self.args, self.config, update_frequency=10, trainFold=self.trainFold
        )
...
if __name__ == "__main__":
    ...
    best_scores = []
    best_thres = []
    for trainFold in range(n_fold):
        print(f"train : {trainFold}")
        ...
        print(f"{trainFold} model_path : {model_path}")
        ...
        print("Best thres is: ", best_thre)
        best_scores.append(best_score)
        best_thres.append(best_thre)
        trainer.clearml_logger.report_scalar(
            "knn_val", "score", iteration=1 + int(trainFold), value=best_score
        )

    average_best_score = sum(best_scores) / len(best_scores)
    print(f"Average best score: {average_best_score}")
    trainer.clearml_logger.report_scalar(
        "knn_val",
        "average_score",
        iteration=1 + int(trainFold),
        value=average_best_score,
    )

    trainval_dataset = HappyWhaleDataset(
        df, transforms=data_transforms["valid"], dummy_label=True
    )
    ...
    outputList = []
    evaluators = [
        Evaluator(args, config, device, model_path) for model_path in model_path_list
    ]
    with torch.no_grad():
        for it, batch in tqdm(enumerate(trainval_loader), total=len(trainval_loader)):
            images = batch["image"]
            images = images.to(device)
            embeddings_list = []
            ...
            outputList.extend(embeddings)

        allTrainData = np.array(outputList)
        allTrainingLabels = np.array(df["individual_id"].values)
    ...
    # TODO
    best_thres = np.mean(best_scores)
    GetSubmission(
        allTrainData,
        ...
        neighbors=100,
        metric="cosine",
        new_individual_thres=best_thre,
