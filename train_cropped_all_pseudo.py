...
ROOT_DIR = "."
TRAIN_DIR = "./input/happy-whale-and-dolphin/20220414/train_fullbody_5k_box"
TEST_DIR = "./input/happy-whale-and-dolphin/20220414/test_fullbody_5k_box"
...
def GetSubmission(
    ...
    for i in range(len(distances)):
        ...
        while True:
            ...
            if preds[i][j] in predictTop:
                j += 1
                continue
            else:
                predictTop.append(preds[i][j])
                topValues.append(conf[i][j])

            j += 1

        predictTop = label_encoder.inverse_transform(predictTop)

        for k in range(len(predictTop)):
            if topValues[k] < new_individual_thres:
                predictTop = np.insert(predictTop, [k], "new_individual")
                break

        if len(predictTop) < 5:
            predictTop = np.append(predictTop, "new_individual")
...
if __name__ == "__main__":
    ...
    trainFold = 1  # this model was trained on fold 0
    data_transforms = get_data_transforms(img_size)

    # df_train = df[df.kfold != trainFold].reset_index(drop=True)
    df_train = df
    # add pseudo
    pseudo_df = pd.read_csv("pseudo_from_33b910f1ef6e4eab96dbdb69d627eecc_0.8.csv")
    pseudo_df["file_path"] = pseudo_df["image"].apply(lambda x: f"{TEST_DIR}/{x}")
    pseudo_df["individual_id"] = label_encoder.transform(pseudo_df["predictions"])
    df_train = pd.concat([df_train, pseudo_df])
    ...
    allTrainData = df_train_cnn_predictions
    allTrainingLabels = train_cnn_labels

    for thre in [0.4, 0.45, 0.5, 0.6]:
        GetSubmission(
            allTrainData,
            ...
            neighbors=100,
            metric="cosine",
            new_individual_thres=thre,
            csv_name=f"./csv/{task.id}_{thre}.csv",
        )
